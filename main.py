    ...
    try:
      ...
      if find:
        if len(find) > 1: # 存在1条以上的记录 则直接返回加入成功
          ...
          return 'Channel error, unable to use: {}'.format(c,_e)
      else:
        return 'Channel error, unable to use: {}'.format(c,_e)
...
async def leave_channel(channel_name):
  ...
  try:
      ...
      return f'no puedo dejar el canal：{channel_name}, {_e}'
...
async def start(event):
  ...
  if 'private_service' in config and config['private_service'] :
    ...
    if chat_id not in authorized_users_list :
        await event.respond('Opps! I\'m a private bot.')
        raise events.StopPropagation
...
async def subscribe(event):
  ...
  if len(splitd) <= 1:
    await event.respond('Input the keyword that needs to subscribe, support JS regular syntax：`/[\s\S]*/ig`')
...
async def unsubscribe_id(event):
  ...
  if len(splitd) > 1:
    ...
    await event.respond('Enter the subscription id of the channel where ** unsubscribe **is required:')
...
async def unsubscribe(event):
  ...
  splitd = [i for i in regex.split('\s+',text) if i]# 删除空元素
  if len(splitd) <= 1:
    await event.respond('Enter a keyword that requires **unsubscribe**')
...
*****Encuentra keywords en los grupos o canales****


El Bot busca keywords en los distintos channels y grupos. 

Usa una Coma (,) para poner mas de un keyword o channel
Utilice espacios entre palabras clave y canales
...
async def common(event):
  ...
  if find:
    ...
    if find['current_status'] == '/subscribe keywords':# 当前输入关键字
      ...
      if len(splitd)  != 3:
        await event.respond('The keyword must not contain Spaces.')
        raise events.StopPropagation
      command, keywords, channels = splitd
